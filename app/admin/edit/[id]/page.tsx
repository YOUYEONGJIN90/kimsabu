...
export default function EditWorkPage() {
  ...
  const [originalWork, setOriginalWork] = useState<WorkPost | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate() || !originalWork) return;
    setSaving(true);
    ...
    <div className="pt-16 md:pt-20 min-h-screen bg-steel-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-steel-900">시공사례 수정</h1>
            <p className="text-steel-500 text-sm">내용을 수정하고 저장하세요</p>
          </div>
        </div>
